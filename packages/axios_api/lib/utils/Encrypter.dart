...
class Encrypter {
  ...
  static final List<int> base64DecodeChars = [
    -1,
    -1,
    -1,
    -1,
  ...
  static String _btoa(String string) {
    ...
    while (i < stringLen) {
      if ((a = codepoint(string, i++)) > 255 ||
          (b = codepoint(string, i++)) > 255 ||
          (c = codepoint(string, i++)) > 255) return "";
      bitmap = (a << 16) | (b << 8) | c;
    ...
    return rest != 0
        ? result.substring(0, result.length - (rest - 3).abs()) +
            "===".substring(rest)
        : result.toString();
  }
  ...
  static String decrypt(String input, [String key = key]) {
    final uriDecoded =
        Uri.decodeComponent(input.replaceAll('"', '')).replaceAll('\\/', '/');
